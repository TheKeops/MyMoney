...
def app():
    ...
    def doviz_function():
            ...
            if _currency == "TRY":
                try:
                    ...
                    messagebox.showerror(
                        "Currency Error",
                        f"An error occurred while loading the exchange rate! Please try again. {e}",
                    )
                ...
                try:
                    ...
                    messagebox.showerror(
                        "Currency Error",
                        "An error occurred while loading the exchange rate! Please try again.",
                    )
        ...
        def hesapmakinesi_function():
            ...
            entry = ctk.CTkEntry(
                root_calc, justify="right", font=("Arial", 24), width=280
            )
            entry.pack(pady=10)
        ...
        name_surname = str(
            name_config["user"]["name"] + " " + name_config["user"]["surname"]
        )
        ...
        title_label_doviz = ctk.CTkLabel(
            root_doviz, text="Currency", font=ctk.CTkFont(size=25, weight="bold")
        )
        title_label_doviz.pack(pady=10)

        background_1 = ctk.CTkFrame(
            root_doviz, width=140, height=160, fg_color="#1A1A1A"
        )
        background_1.pack(pady=10)
        background_1.place(x=50, y=70)

        background_2 = ctk.CTkFrame(
            root_doviz, width=140, height=160, fg_color="#1A1A1A"
        )
        background_2.pack(pady=10)
        background_2.place(x=220, y=70)

        background_3 = ctk.CTkFrame(
            root_doviz, width=140, height=160, fg_color="#1A1A1A"
        )
        ...
        currency_label_1 = ctk.CTkLabel(
            root_doviz, text=f"No Data", font=ctk.CTkFont(size=18), bg_color="#1A1A1A"
        )
        currency_label_1.pack()
        currency_label_1.place(x=80, y=140)

        currency_label_2 = ctk.CTkLabel(
            root_doviz, text=f"No Data", font=ctk.CTkFont(size=18), bg_color="#1A1A1A"
        )
        currency_label_2.pack()
        currency_label_2.place(x=250, y=140)

        currency_label_3 = ctk.CTkLabel(
            root_doviz, text=f"No Data", font=ctk.CTkFont(size=18), bg_color="#1A1A1A"
        )
        ...
        hesapmakinesi = ctk.CTkButton(
            root_doviz,
            text="HESAP MAKİNESİ",
            font=("century gothic", 16, "bold"),
            command=hesapmakinesi_function,
        )
        hesapmakinesi.pack()
        ...
        date_label = ctk.CTkLabel(
            root_doviz, text=f"Loading...", font=("century gothic", 14, "bold")
        )
        date_label.pack()
        date_label.place(x=10, y=370)

        name_surname_label = ctk.CTkLabel(
            root_doviz, text=f"{name_surname}", font=("century gothic", 15, "bold")
        )
    ...
    def gelir_gider_function():
        ...
        def open_all_saves():
            ...
            def open_save_data():
                ...
                def load_save_currency_data():
                    ...
                    with open(
                        f"app/saves/{selected_save_file}/{txt_files[0]}",
                        "r",
                        encoding="utf-8",
                    ) as f:
                        lines = f.readlines()
                ...
                def delete_save():
                    ...
                    if messagebox.askyesno(
                        ...
                        messagebox.showinfo(
                            "Successful", "The record has been successfully deleted."
                        )
                        root_osd.destroy()
                        root_oas.destroy()
                ...
                with open(
                    f"app/saves/{selected_save_file}/data-info.json",
                    "r",
                    encoding="utf-8",
                ) as f:
                    data = json.load(f)
                ...
                title_osd = ctk.CTkLabel(
                    root_osd,
                    text="Kayıt Detayları",
                ...
                reload_button = ctk.CTkButton(
                    ...
                    text="Yenile",
                    font=("century gothic", 13, "bold"),
                    command=load_save_currency_data,
                )
                reload_button.pack()
            ...
            title_oas = ctk.CTkLabel(
                root_oas,
                text="Tüm Para Kayıtları",
                font=ctk.CTkFont(size=25, weight="bold"),
            )
            ...
            open_save = ctk.CTkButton(
                root_oas,
                text="Kayıdı Aç",
                font=("century gothic", 17, "bold"),
                command=open_save_data,
            )
        ...
        def save():
            if (
                available_money_currency.get().upper().strip() == ""
                or available_money_currency_to.get().upper().strip() == ""
                or save_names_entry.get().lower().strip() == ""
                or available_money.get().lower().strip() == ""
            ):
                root_gg.attributes("-topmost", False)
                ...
                currency_save_to = str(
                    available_money_currency_to.get().upper().strip()
                )

                save_currency_saves.save_currency_saves(
                    file_name=str(save_name),
                    currency=currency_save,
                    currency_to=currency_save_to,
                    available_money=float(money),
                )
                currency_updater_bot.start_currency_update()

                root_gg.attributes("-topmost", False)
                messagebox.showinfo(
                    "Successful", "Your registration has been saved successfully!"
                )
                root_gg.attributes("-topmost", True)
        ...
        def update_currency():
            ...
            root_gg.attributes("-topmost", False)
            messagebox.showinfo(
                "Successful", "All currencies have been updated successfully!"
            )
            root_gg.attributes("-topmost", True)
        ...
        title_gg = ctk.CTkLabel(
            root_gg, text="Para Kayıt", font=ctk.CTkFont(size=25, weight="bold")
        )
        title_gg.pack(pady=10)

        available_money_currency = ctk.CTkComboBox(
            root_gg, values=("USD", "TRY", "EUR", "GBP")
        )
        available_money_currency.pack(pady=10)
        available_money_currency.set("Para Türü.")

        available_money_currency_to = ctk.CTkComboBox(
            root_gg, values=("USD", "TRY", "EUR", "GBP")
        )
        available_money_currency_to.pack(pady=10)
        available_money_currency_to.set("Çevirilecek.")

        available_money = ctk.CTkEntry(
            root_gg, placeholder_text="Para değer giriniz...", width=170
        )
        available_money.pack(pady=10)

        save_names_entry = ctk.CTkEntry(
            root_gg, placeholder_text="Kayıt adını giriniz...", width=170
        )
        save_names_entry.pack(pady=10)

        saves_save = ctk.CTkButton(
            root_gg, text="Kaydet", font=("century gothic", 13, "bold"), command=save
        )
        saves_save.pack(pady=15)

        all_saves = ctk.CTkButton(
            root_gg,
            text="Tüm Kayıtlar",
            font=("century gothic", 13, "bold"),
            command=open_all_saves,
        )
        all_saves.pack(pady=10)

        currency_update = ctk.CTkButton(
            root_gg,
            text="Kurları Güncelle",
            font=("century gothic", 13, "bold"),
            command=update_currency,
            fg_color="#ffb700",
            hover_color="#7e5b01",
        )
    ...
    def analiz_function():
        def start_analysis():
            def load_analysis_data():
                with open(
                    f"app/saves/{save_file}/data-info.json", "r", encoding="utf-8"
                ) as f:
                ...
                for i in os.listdir(os.path.abspath(f"app/saves/{save_file}")):
                    if i.endswith(".txt"):
                        with open(
                            f"app/saves/{save_file}/{i}", "r", encoding="utf-8"
                        ) as f:
                            lines = f.readlines()
            ...
            def grafik_analizi():
                ...
                messagebox.showinfo("Info", "This field will be added in the future...")
                root_analysis_result.attributes("-topmost", True)

            def machine_learning(in_function=False):
                global pred
                ...
                X = np.arange(len(money_list)).reshape(-1, 1)
                y = np.array(money_list)

                model = LinearRegression()
                model.fit(X, y)

                print(money_data_list_analysis.size())

                future = np.array(
                    [
                        [money_data_list_analysis.size() - 2],
                        [money_data_list_analysis.size() - 1],
                        [money_data_list_analysis.size()],
                    ]
                )
                ...
                if in_function == False:
                    ...
                    messagebox.showinfo(
                        "Machine Result",
                        f"Machine estimated results. (Warning! We recommend that you do not make any investments based on this information. We disclaim any liability.)",
                    )
                    messagebox.showinfo(
                        "Machine Result",
                        f"Results : {str(pred).strip().replace(' ', ',')}",
                    )
                    root_analysis_result.attributes("-topmost", True)
                else:
                    pred = model.predict(future)
                    str(pred).strip().replace(" ", ",")

            def report_screen():
                def report_generate():
                    if report_tic.get() == True:
                        machine_learning(in_function=True)
                    else:
                        pass
                    try:
                        report_name_val = report_name.get()
                        analysis_type = analysis_type_combobox.get().strip().upper()
                        machine_learn = report_tic.get()

                        note = add_notes.get("1.0", tk.END).strip()
                        analysis_insert = f""
                        machine_insert = f"{pred} * Bu sonuçlara dayanarak herhangi bir yatırım yapmamanız tavsiye edilir, aksi halde sorumluluk bize ait değildir. Bu bilgiler sadece size bilgilendirmek amaçlı ve tahmin üzerine dayalıdır."

                        if (analysis_type == "KAR ANALIZ" or analysis_type == "KAR ANALİZ"):
                            insert_filtre(in_function=True, type="Kar Analiz")
                            analysis_insert = f"Kar Analizi Sonucu : {kar_analiz_data}"

                        elif (analysis_type == "SON 5 VERI" or analysis_type == "SON 5 VERİ"):
                            insert_filtre(in_function=True, type="Son 5 Analiz")
                            analysis_insert = f"Son 5 Veri Analiz Sonucu : {son_5_data}"

                        elif (analysis_type == "SON 2 VERI" or analysis_type == "SON 2 VERİ"):
                            insert_filtre(in_function=True, type="Son 2 Analiz")
                            analysis_insert = f"Son 2 Veri Analiz Sonucu : {son_2_data}"
                        else:
                            pass

                        save_path_ = os.path.abspath(f"{save_file}")

                        create_files_module.create_paper(
                            report_name=report_name_val,
                            report_notes=note,
                            machine=machine_learn,
                            analysis_type=analysis_type,
                            analysis_results=analysis_insert,
                            machine_results=machine_insert,
                            save_path=save_path_,
                        )
                        report_root.attributes("-topmost", False)
                        messagebox.showinfo(
                            "Successful", "Your report has been created successfully!"
                        )
                ...
                report_root = ctk.CTkToplevel()
                report_root.geometry("300x530")
                report_root.title("MyMoney - Rapor Oluştur")
                report_root.resizable(False, False)
                root_analysis_result.attributes("-topmost", False)
                report_root.attributes("-topmost", True)

                x = (report_root.winfo_screenwidth() - 300) // 2
                y = (report_root.winfo_screenheight() - 530) // 2
                report_root.geometry(f"+{x}+{y}")

                title_report = ctk.CTkLabel(
                    report_root,
                    text="Raport Oluştur",
                    font=("century gothic", 20, "bold"),
                )
                title_report.pack(pady=15)

                report_name = ctk.CTkEntry(
                    report_root, placeholder_text="Rapor Adını Giriniz... *", width=200
                )
                report_name.pack(pady=10)

                report_tic = ctk.BooleanVar()

                machine_learning_adding_report = ctk.CTkCheckBox(
                    report_root,
                    text="Makina Tahmini Ekle",
                    font=("century gothic", 16, "bold"),
                    variable=report_tic,
                )
                machine_learning_adding_report.pack(pady=10)
                report_tic.set(1)

                analysis_type_combobox = ctk.CTkComboBox(
                    report_root, values=("Kar Analiz", "Son 5 Veri", "Son 2 Veri")
                )
                analysis_type_combobox.pack(pady=10)
                ...
                add_notes_label.pack(pady=10)

                add_notes = ctk.CTkTextbox(report_root)
                add_notes.pack(pady=10)

                create_report = ctk.CTkButton(
                    report_root,
                    text="Oluştur",
                    font=("century gothic", 16, "bold"),
                    command=report_generate,
                )
                create_report.pack(pady=10)

                report_root.mainloop()

            def insert_filtre(in_function=False, type=None):
                global kar_analiz_data, son_2_data, son_5_data
                filter_type = analysis_filter_combobox.get()

                if in_function == True:
                    filter_type = str(type)
                else:
                    global kar_analiz_data, son_2_data, son_5_data
                    filter_type = analysis_filter_combobox.get()
                ...
                if filter_type == "Son 5 Analiz":
                    ...
                    last_index = money_data_list_analysis.size() - 1
                    five_index = money_data_list_analysis.size() - 5

                    last_data_money = (
                        money_data_list_analysis.get(last_index)
                        .split("-")[0]
                        .strip()
                        .split(" ")[0]
                    )
                    five_data_money = (
                        money_data_list_analysis.get(five_index)
                        .split("-")[0]
                        .strip()
                        .split(" ")[0]
                    )

                    if float(five_data_money) < float(last_data_money):
                        info_label_analysis.configure(
                            text=f"▲ {abs(round((float(five_data_money) - float(last_data_money)), 2))} {data_analysis['currency']}"
                        )

                        son_5_data = f"{abs(round((float(five_data_money) - float(last_data_money)), 2))} {data_analysis['currency']}"

                        root_analysis_result.update()
                    elif float(five_data_money) > float(last_data_money):
                        info_label_analysis.configure(
                            text=f"▼ {round((float(five_data_money) - float(last_data_money)), 2)} {data_analysis['currency']}"
                        )

                        son_5_data = f"{round((float(five_data_money) - float(last_data_money)), 2)} {data_analysis['currency']}"

                        root_analysis_result.update()
                    else:
                        info_label_analysis.configure(
                            text=f"NOTR (0 {data_analysis['currency']})"
                        )

                        son_5_data = f"NOTR (0 {data_analysis['currency']})"
                    ...
                    first_data_money = (
                        money_data_list_analysis.get(0)
                        .split("-")[0]
                        .strip()
                        .split(" ")[0]
                    )
                    ...
                    currency_pair = f"{data_analysis['currency']}={data_analysis['to']}"
                    currency_str = getdata_module.get_currency_data(currency_pair)

                    currency_clean = "".join(
                        ch for ch in str(currency_str) if ch.isdigit() or ch == "."
                    )
                    money_data = float(currency_clean)

                    first_data = float(first_data_money) * money_data
                    last_data = float(last_data_money) * money_data

                    if float(first_data) < float(last_data):
                        info_label_analysis.configure(
                            text=f"▲ {abs(round((float(first_data) - float(last_data)), 2))} {data_analysis['to']}"
                        )

                        kar_analiz_data = f"{abs(round((float(first_data) - float(last_data)), 2))} {data_analysis['to']}"

                        root_analysis_result.update()

                    elif float(first_data) > float(last_data):
                        info_label_analysis.configure(
                            text=f"▼ {round((float(first_data) - float(last_data)), 2)} {data_analysis['to']}"
                        )

                        kar_analiz_data = f"{round((float(first_data) - float(last_data)), 2)} {data_analysis['to']}"

                        root_analysis_result.update()
                    else:
                        info_label_analysis.configure(
                            text=f"NOTR (0 {data_analysis['to']})"
                        )

                        kar_analiz_data = f"NOTR (0 {data_analysis['to']})"
                    ...
                    one_index = money_data_list_analysis.size() - 1
                    two_index = money_data_list_analysis.size() - 2

                    one_data_money = (
                        money_data_list_analysis.get(one_index)
                        .split("-")[0]
                        .strip()
                        .split(" ")[0]
                    )
                    two_data_money = (
                        money_data_list_analysis.get(two_index)
                        .split("-")[0]
                        .strip()
                        .split(" ")[0]
                    )

                    if float(one_data_money) < float(two_data_money):
                        info_label_analysis.configure(
                            text=f"▲ {abs(round((float(one_data_money) - float(two_data_money)), 2))} {data_analysis['currency']}"
                        )

                        son_2_data = f"{abs(round((float(one_data_money) - float(two_data_money)), 2))} {data_analysis['currency']}"

                        root_analysis_result.update()
                    elif float(one_data_money) > float(two_data_money):
                        info_label_analysis.configure(
                            text=f"▼ {round((float(one_data_money) - float(two_data_money)), 2)} {data_analysis['currency']}"
                        )
                        ...
                        son_2_data = f"NOTR (0 {data_analysis['currency']})"
            ...
            if not analysis_type == "Gelir-Gider Grafik":
                ...
                root_analysis_result = ctk.CTkToplevel(root)
                ...
                title_label_analysis_result = ctk.CTkLabel(
                ...
                money_data_list_analysis = tk.Listbox(
                    root_analysis_result,
                    width=27,
                    height=15,
                    border=0,
                ...
                reload_button_analysis = ctk.CTkButton(
                    root_analysis_result,
                    text="Yenile",
                    font=("century gothic", 13, "bold"),
                    command=reload_analysis_data,
                )
                ...
                info_label_analysis = ctk.CTkLabel(
                    root_analysis_result,
                ...
                analysis_filter_combobox = ctk.CTkComboBox(
                    root_analysis_result,
                    values=("Son 5 Analiz", "Kar Analiz", "Son 2 Analiz"),
                    width=150,
                )
                ...
                insert_filter_button = ctk.CTkButton(
                    root_analysis_result,
                    text="Filtrele",
                    font=("century gothic", 13, "bold"),
                    fg_color="#ed0000",
                    hover_color="#7e0101",
                    command=insert_filtre,
                )
                insert_filter_button.pack()
                insert_filter_button.place(x=440, y=90)

                grafik_analiz_button = ctk.CTkButton(
                    root_analysis_result,
                    text="Grafik Analiz",
                    font=("century gothic", 13, "bold"),
                    command=grafik_analizi,
                )
                grafik_analiz_button.pack()
                grafik_analiz_button.place(x=360, y=150)

                machine_learning_button = ctk.CTkButton(
                    root_analysis_result,
                    text="Makine Analizi",
                    font=("century gothic", 13, "bold"),
        ...
        title_label_analysis = ctk.CTkLabel(
            root_analysis, text="Analiz", font=ctk.CTkFont(size=25, weight="bold")
        )
        title_label_analysis.pack(pady=10)

        saves_combobox = ctk.CTkComboBox(
            root_analysis, values=os.listdir(os.path.abspath("app/saves")), width=200
        )
        saves_combobox.pack(pady=20)
        saves_combobox.set("Kayıt Seçiniz.")

        analyze_type_combobox = ctk.CTkComboBox(
            root_analysis, values=["Gelir-Gider Grafik"], width=200
        )
    ...
    def ayarlar_function():
        ...
        def rename_function():
            ...
            def save_name(name, surname):
                if name_entry.get() == "" or surname_entry.get() == "":
                    ...
                    messagebox.showinfo(
                        "Success", "Name and Surname changed successfully."
                    )
            ...
            title_label_rename = ctk.CTkLabel(
                root_settings_rename,
                text="Change Name",
                font=ctk.CTkFont(size=20, weight="bold"),
            )
            ...
            name_label = ctk.CTkLabel(
                root_settings_rename, text="Name", font=("Century Gothic", 16, "bold")
            )
            name_label.pack()
            ...
            surname_entry = ctk.CTkEntry(
                root_settings_rename, font=ctk.CTkFont(size=14)
            )
            ...
            enter = ctk.CTkButton(
                ...
                font=("Century Gothic", 14, "bold"),
                width=100,
                command=lambda: save_name(name_entry.get(), surname_entry.get()),
            )
            enter.pack(pady=10)
        ...
        def save_settings():
            try:
                ...
                messagebox.showinfo(
                    "Success",
                    "All your settings will be saved, but the app will close and you will need to reopen it.",
                )
        ...
        title_label_settings = ctk.CTkLabel(
            root_settings, text="Settings", font=ctk.CTkFont(size=25, weight="bold")
        )
        title_label_settings.pack(pady=10)

        theme_label = ctk.CTkLabel(
            root_settings, text="Theme", font=("Century Gothic", 16, "bold")
        )
        theme_label.pack()

        theme_combobox = ctk.CTkComboBox(
            root_settings, values=["Light", "Dark", "System"], font=ctk.CTkFont(size=14)
        )
        theme_combobox.set(configure["theme"].capitalize())
        theme_combobox.pack(pady=10)

        language_label = ctk.CTkLabel(
            root_settings, text="Language", font=("Century Gothic", 16, "bold")
        )
        language_label.pack()

        language_combobox = ctk.CTkComboBox(
            root_settings, values=["English", "Turkish"], font=ctk.CTkFont(size=14)
        )
        language_combobox.set("English" if configure["language"] == "en" else "Turkish")
        language_combobox.pack(pady=10)

        currency_label = ctk.CTkLabel(
            root_settings, text="Currency", font=("Century Gothic", 16, "bold")
        )
        currency_label.pack()

        currency_combobox = ctk.CTkComboBox(
            root_settings, values=["USD", "TRY"], font=ctk.CTkFont(size=14)
        )
        ...
        auto_refresh_box = ctk.CTkCheckBox(
            root_settings,
            text="Auto Refresh",
            variable=tik_deger,
            command=checkbox_auto_refresh_function,
            font=("Century Gothic", 14, "bold"),
        )
        ...
        github_link.bind(
            "<Button-1>",
            lambda e: webbrowser.open_new("https://github.com/TheKeops/MyMoney"),
        )
        ...
        version_label_settings = ctk.CTkLabel(
            root_settings,
            text=f"Version {configure['version']}",
    ...
    if (
        configure["user"]["name"] == "None"
        or configure["user"]["surname"] == "None"
        or configure["user"]["name"] == ""
        or configure["user"]["surname"] == ""
    ):
    ...
    title_label = ctk.CTkLabel(
        root,
        text=f"Hi, {configure['user']['name']}",
        font=ctk.CTkFont(size=20, weight="bold"),
    )
    title_label.pack(pady=10)

    doviz_button = ctk.CTkButton(
        root,
        text="Currency",
        ...
        hover_color="#A49100",
        width=250,
        command=doviz_function,
    )
    doviz_button.pack(pady=10)

    gelir_gider_button = ctk.CTkButton(
        root,
        ...
        hover_color="#A40000",
        width=250,
        command=gelir_gider_function,
    )
    gelir_gider_button.pack(pady=10)

    analiz_button = ctk.CTkButton(
        root,
        text="Analysis",
        font=("Century Gothic", 18, "bold"),
        width=250,
        command=analiz_function,
    )
    analiz_button.pack(pady=10)

    ayarlar_button = ctk.CTkButton(
        root,
        text="Settings",
        font=("Century Gothic", 18, "bold"),
        fg_color="#434343",
        hover_color="#858585",
        width=250,
        command=ayarlar_function,
    )
    ayarlar_button.pack(pady=10)

    version_label = ctk.CTkLabel(
        root, text=f"Version {configure['version']}", font=("Century Gothic", 14)
    )
...
def splash_screen():
    ...
    try:
        ...
        tk.Label(
            splash, text="MyMoney Uygulaması Yükleniyor...", font=("Arial", 16)
        ).pack(expand=True)
