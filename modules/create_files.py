...
def create_paper(machine=False, analysis_type="Kar Analizi", report_name="DEFAULT_REPORT", analysis_results=None, machine_results=None, report_notes="Herhangi bir raport notu eklenmedi."):
    ...
    try:
        ...
        open(f"{desktop}/{str(report_name).strip().lower()}-report.txt", 'x', encoding='utf-8').close()
    except:
        pass

    with open(f"{desktop}/{str(report_name).strip().lower()}-report.txt", 'w', encoding='utf-8') as f:
        f.writelines(f"MyMoney Raporu ({report_name})\n")
        f.writelines("====================================\n\n")

        with open("app/settings.json", 'r', encoding='utf-8') as settings_file:
            settings = json.load(settings_file)
            f.writelines(f"Kullanıcı Adı: {settings['user']['name']} {settings['user']['surname']}\n")
            f.writelines(f"Tarih: {str(datetime.datetime.now().strftime('%d/%m/%Y %H:%M:%S'))}\n")
        ...
        f.writelines("* Yukarıda verilen bilgilere ait kullanıcı tarafından MyMoney uygulaması üzerinden talep edilen raporun detayları aşağıda belirtilmiştir.\n\n")
