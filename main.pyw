        ...
        def start_analysis():
            ...
            def report_screen():
                def report_generate():
                    ...
                    create_files_module.create_paper(report_name=report_name_val, report_notes=note, machine=machine_learn)
                ...
                title_report = ctk.CTkLabel(report_root, text="Raport Oluştur", font=("century gothic",20,"bold"))
                title_report.pack(pady=15)

                report_name = ctk.CTkEntry(report_root, placeholder_text="Rapor Adını Giriniz... *", width=200)
                report_name.pack(pady=10)

                report_tic = ctk.BooleanVar()

                machine_learning_adding_report = ctk.CTkCheckBox(report_root, text="Makina Tahmini Ekle", font=("century gothic",16,"bold"), variable=report_tic)
                machine_learning_adding_report.pack(pady=10)
                report_tic.set(1)

                analysis_type_combobox = ctk.CTkComboBox(report_root, values=("Kar Analiz","Son 5 Veri","Son 2 Veri"))
                analysis_type_combobox.pack(pady=10)

                add_notes_label = ctk.CTkLabel(report_root, text="Not (Opsiyonel)", font=("century gothic",15,"bold"))
                add_notes_label.pack(pady=10)

                add_notes = ctk.CTkTextbox(report_root)
                add_notes.pack(pady=10)

                create_report = ctk.CTkButton(report_root,text="Oluştur", font=("century gothic",16,"bold"), command=report_generate)
                        ...
                        if i.endswith(".txt"):
                            with open(f"app/saves/{save_file}/{i}","r",encoding="utf-8") as f:
                                ...
                                for j in f.readlines():
                                    date = str(j).strip().split("-")
                                    date_list.append(date[1].strip())

                                plt.figure(figsize=(10,5))
                                plt.plot(money_list, color='blue', linestyle='-')
                                plt.title(f"{save_file} Gelir-Gider Grafiği")
                                plt.xlabel("Kayıt Sayısı")
                                plt.ylabel(f"Para ({data_analysis['currency']})")
                                plt.grid()
                                plt.tight_layout()
                                plt.gcf().canvas.manager.set_window_title("MyMoney - Gelir-Gider Grafiği")
                                plt.show()
                except:
                    pass
            ...
            def machine_learning():
                ...
                messagebox.showinfo("Machine Result",f"Machine estimated results. (Warning! We recommend that you do not make any investments based on this information. We disclaim any liability.)")
                messagebox.showinfo("Machine Result",f"Results : {pred}")
            ...
            def insert_filte():
                ...
                if filter_type == "Son 5 Analiz":
                    ...
                    last_data_money = money_data_list_analysis.get(last_index).split("-")[0].strip().split(" ")[0]
                    five_data_money = money_data_list_analysis.get(five_index).split("-")[0].strip().split(" ")[0]

                    if float(five_data_money) < float(last_data_money):
                        info_label_analysis.configure(text=f"▲ {abs(round((float(five_data_money) - float(last_data_money)),2))} {data_analysis['currency']}")
                        root_analysis_result.update()
                    elif float(five_data_money) > float(last_data_money):
                        info_label_analysis.configure(text=f"▼ {round((float(five_data_money) - float(last_data_money)),2)} {data_analysis['currency']}")
                        root_analysis_result.update()
                    else:
                        info_label_analysis.configure(text=f"NOTR (0 {data_analysis['currency']})")
                        root_analysis_result.update()
                    ...
                    currency_clean = ''.join(ch for ch in str(currency_str) if ch.isdigit() or ch == '.')
                    ...
                    if float(first_data) < float(last_data):
                        info_label_analysis.configure(text=f"▲ {abs(round((float(first_data) - float(last_data)),2))} {data_analysis['to']}")
                        root_analysis_result.update()
                    elif float(first_data) > float(last_data):
                        info_label_analysis.configure(text=f"▼ {round((float(first_data) - float(last_data)),2)} {data_analysis['to']}")
                        root_analysis_result.update()
                    else:
                    ...
                    one_data_money = money_data_list_analysis.get(one_index).split("-")[0].strip().split(" ")[0]
                    two_data_money = money_data_list_analysis.get(two_index).split("-")[0].strip().split(" ")[0]

                    if float(one_data_money) < float(two_data_money):
                        info_label_analysis.configure(text=f"▲ {abs(round((float(one_data_money) - float(two_data_money)),2))} {data_analysis['currency']}")
                        root_analysis_result.update()
                    elif float(one_data_money) > float(two_data_money):
                        info_label_analysis.configure(text=f"▼ {round((float(one_data_money) - float(two_data_money)),2)} {data_analysis['currency']}")
                        root_analysis_result.update()
                    else:
                        info_label_analysis.configure(text=f"NOTR (0 {data_analysis['currency']})")
                        root_analysis_result.update()
            ...
            if not analysis_type == "Gelir-Gider Grafik":
                ...
                analysis_filter_combobox = ctk.CTkComboBox(root_analysis_result, values=("Son 5 Analiz","Kar Analiz","Son 2 Analiz"), width=150)
                ...
                machine_learning_button = ctk.CTkButton(root_analysis_result, text="Makine Analizi", font=("century gothic",13,"bold"), command=machine_learning)
                machine_learning_button.pack()
                machine_learning_button.place(x=360, y=210)

                rapor_button = ctk.CTkButton(root_analysis_result, text="Rapor Oluştur", font=("century gothic",13,"bold"), command=report_screen)
                rapor_button.pack()
